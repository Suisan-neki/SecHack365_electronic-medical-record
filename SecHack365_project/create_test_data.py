...
def main():
    ...
    print(f"[INFO] ユーザー {username} の暗号化キーを取得中...")
    ...
    print(f"[SUCCESS] 暗号化キーを生成: {len(encryption_key)}バイト")
            ...
            'medical_records': [
                ...
                },
                {
                    ...
                    'data': {
                        'diagnosis': '定期健診',
                        'medication': 'なし',
                        'notes': '全体的に健康状態は良好。軽度の貧血が見つかったため経過観察。',
                        'doctor': 'Dr. 佐藤',
                        'blood_pressure': '118/78',
                        'temperature': '36.3°C'
                    },
                    'signature': 'yamashita_signature_2'
                }
            ]
        }
    }
    ...
    print(f"[SUCCESS] 暗号化データを保存: {output_file}")
    print(f"[INFO] 患者データ:")
    print(f"  - 患者ID: P001")
    print(f"  - 患者名: 山下真凜")
    print(f"  - 診断記録: 2件")
    print(f"  - 暗号化方式: {encryptor.get_encryption_info()['encryption_type']}")
    ...
    try:
        ...
        if test_decrypted == patient_data:
            print("[SUCCESS] 復号テスト成功 - データの整合性を確認")
            return True
        else:
            print("[ERROR] 復号テスト失敗 - データが一致しません")
            return False
...
if __name__ == "__main__":
    ...
    if success:
        print("\n✅ 患者データの作成が完了しました")
        print("アプリケーションで「患者情報を抽出」をテストしてください")
    else:
        print("\n❌ 患者データの作成に失敗しました")
