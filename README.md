...
### 2024年9月27日 - 中間発表完了 ✅
- 電子カルテシステムの基本機能を実装
- デジタル署名機能（RSA 2048bit + PSS padding + SHA-256）
- ハッシュチェーンによる監査ログの整合性検証
- 認証システム（TOTP + WebAuthn対応）
- Webインターフェース（医師向け管理画面・患者向け表示画面）
- セキュリティ検証ダッシュボード

## 模擬電子カルテシステム

### 概要
既存の電子カルテシステムを模擬したデモ用システムです。メインシステムとのデータ連携を実証するために開発されました。

### 起動方法

```bash
# 模擬電子カルテシステムのディレクトリに移動
cd dummy_ehr_system
...
- **患者管理**: 患者一覧・詳細表示
- **診療記録管理**: 診療記録の一覧・詳細表示
- **データ抽出**: CSV/FHIR形式でのデータエクスポート
- **データ受信**: メインシステムからの診療記録受信
- **API連携**: メインシステムとの双方向データ連携

### データ連携フロー

1. **患者選択**: メインシステムで「患者情報を抽出」をクリック
2. **データ取得**: 模擬カルテから患者データを取得
3. **診察実施**: メインシステムで診療記録を入力
4. **データ送信**: 診療記録を模擬カルテに送信
5. **記録確認**: 模擬カルテで診療記録を確認
